...
namespace RssAggregator.Application.Tests.Posts.GetUserPosts;
...
public class GetUserPostsUseCaseShould
{
    ...
    public async Task ReturnPosts_WhenAllGood()
    {
        ...
        _identity.UserId.Returns(Guid.Parse("F9F40938-1954-4938-B8D2-534C790BE9A8")); // TODO: should be smth like _identityProvider.Current.IsAuthenticated().Returns(true);
        _storage
    ...
    {
        var request = new GetUserPostsRequest(new TestSpecification());
        _identity.UserId.Returns(Guid.Empty); // TODO: should be smth like _identityProvider.Current.IsAuthenticated().Returns(false);
