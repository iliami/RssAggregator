    ...
    public void Configure(EntityTypeBuilder<Feed> builder)
    {
        ...
        builder.HasMany(x => x.Subscribers).WithMany()
            .UsingEntity("Subscriptions",
                r => r.HasOne(typeof(AppUser)).WithMany().HasForeignKey("AppUserId").HasPrincipalKey(nameof(AppUser.Id)),
                l => l.HasOne(typeof(Feed), nameof(Feed.Subscribers)).WithMany().HasForeignKey("FeedId").HasPrincipalKey(nameof(Feed.Id)),
